...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get settings for dynamic threshold
    ...
    const hasIndividualRates = products.some(
      (item) =>
        item.individualShippingRate !== null &&
        item.individualShippingRate !== undefined
    );
    ...
    const hasExpressItems = products.some(
      (item) => item.shippingClass === "EXPRESS"
    );
    const hasFragileItems = products.some(
      (item) => item.shippingClass === "FRAGILE"
    );
    ...
    return NextResponse.json({
      ...
      breakdown: {
        ...
        totalWeight,
        itemCount: items.reduce((sum, item) => sum + item.quantity, 0),
        baseShipping: hasIndividualRates
          ? shippingCost -
            (hasExpressItems ? 20 : 0) -
            (hasFragileItems ? 15 : 0)
          : calculateShippingCost(totalWeight, products, address),
