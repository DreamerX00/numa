...
import { generateInvoicePDF, generateInvoiceHTML } from '@/lib/invoice/generator';
...
export async function POST(
  ...
  try {
    ...
    if (!invoice || regenerate) {
      ...
      const customerState = order.shippingAddress?.state?.toUpperCase() || order.billingAddress?.state?.toUpperCase() || "";
      ...
      // Company information (this should ideally come from settings)
      const companyInfo = {
        name: "Numa Jewelry",
        address: "123 Business Street, Mumbai, Maharashtra 400001",
        gstin: "27ABCDE1234F1Z5", // This should come from settings
        phone: "+91-9876543210",
        email: "contact@numa.com",
        website: "www.numa.com",
        state: "MAHARASHTRA"
      };
      ...
      const invoiceData = {
        ...
        customerPhone: order.user.profile?.phone || order.shippingAddress?.phone,
        billingAddress: order.billingAddress ? JSON.parse(JSON.stringify(order.billingAddress)) : (order.shippingAddress ? JSON.parse(JSON.stringify(order.shippingAddress)) : null),
